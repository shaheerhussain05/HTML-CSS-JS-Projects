...
function renderIncorrectLetters() {
    ...
    hangmanParts.forEach( (part, index) => {
        ...
        // check if the index value of the hangman part is less than the number of incoorrect letters
    ...
    if ( incorrectLetters.length === hangmanParts.length ) { 
        // if true , set the game over message
...
window.addEventListener('keydown', e => {
    // check if the keyboard key pressesd is a letter
    if ( e.keyCode >= 65 && e.keyCode <= 90 ) {
        // if the keyCode is between 65 and 90 , save the letter
        const letter = e.key;
        // check to see if the letter is in the randomWord
        if ( randomWord.includes(letter) ) {
            // if the randomWord has the letter, check to see if letter is already in correct letters
            ...
            // if the randomWord does not have the letter, check to see if letter is already in the incorrect letters array
...
// listen for click on the playbtn
playBtn.addEventListener('click', () => {
    ...
    renderWord();
    // update the incorrect letters sections
    renderIncorrectLetters();
